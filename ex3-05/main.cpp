...
int main() {
  // Function with real root: f(x) = x^2 - 1 = 0.
  {
    ...
    const std::function<double(double)> f = [](const double &x) { return x * x - 1; };
    const std::function<double(double)> df = [](const double &x) { return 2 * x; };
  ...
  // Function with complex root: f(x) = x^2 +1 = 0.
  {
    ...
    if (root != std::numeric_limits<double>::quiet_NaN()) {
      std::cout << "Approximate root comlex root: " << root << std::endl;
    } else {
      std::cout << "Failed to converge to a root." << std::endl;
    }
  }
